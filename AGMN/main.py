...
from mymodel.lexicon_augment_ner import AGMN as AGMNModel
from enutils.data import Data
import warnings
...
def evaluate(data, model, name):
    ...
    for batch_id in range(total_batch):
        with torch.no_grad():
            ...
            if not instance:
                continue
            gaz_list, batch_word, batch_wordlen, batch_label, layer_gaz, gaz_count, gaz_chars, gaz_mask, gazchar_mask, mask, batch_bert, bert_mask = batchify_with_label(
                instance, data.HP_gpu, data.HP_num_layer, True)
            tag_seq, gaz_match = model(gaz_list, batch_word, batch_wordlen, layer_gaz, gaz_count,
    ...
    x=0.00000001
    speed = len(instances) / (decode_time+x)
...
def batchify_with_label(input_batch_list, gpu, num_layer, volatile_flag=False):
    ...
    for b, (seq, bert_id,  label, seqlen, layergaz, gazmask, gazcount, gazchar, gazchar_mask, gaznum,
            gazlen) in enumerate(
        zip(words, bert_ids, labels, word_seq_lengths, layer_gazs, gaz_mask, gaz_count, gaz_chars,
    ...
    return gazs, word_seq_tensor, word_seq_lengths, label_seq_tensor, layer_gaz_tensor, gaz_count_tensor, gaz_chars_tensor, gaz_mask_tensor, gazchar_mask_tensor, mask, bert_seq_tensor, bert_mask


def train(data, save_model_dir, seg=True):


    model = AGMNModel(data)
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            instance_count += 1
            loss, tag_seq = model.neg_log_likelihood_loss(gaz_list, batch_word, batch_wordlen, layer_gaz,
        ...
        if current_score > best_dev:
            if seg:
                print("Exceed previous best f score:", f)

            else:
                print("Exceed previous best acc score:", f)
        ...
        print("Best dev score: p:{}, r:{}, f:{}".format(best_dev_p, best_dev_r, f))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dev', default="data/msra/dev.char.bmes")
    parser.add_argument('--test', default="data/msra/test.char.bmes")
    parser.add_argument('--dataset', default='msra', help='weibo|resume|ontonotes|msra')
    ...
    parser.add_argument('--ff_dropout_2', default=0.4, type=float)
    ...
    word_img = "E:/data/Chinese-Character-Feature-Resource-master/image"
    char_emb="E:/data/gigaword_chn.all.a2b.uni.ite50.vec"
    # bichar_emb = "cpt/gigaword_chn.all.a2b.bi.ite50.vec"
    gaz_file = "E:/data/tencent-ailab-embedding-zh-d200-v0.2.0-s.txt"
    sys.stdout.flush()
    ...
    if status == 'train':
            ...
            data.generate_img_weight(word_img)
